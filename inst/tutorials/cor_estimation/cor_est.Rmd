...
date: "`r Sys.Date()`"
description: "Estimate correlation coefficient with trial & error"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
res <- knitr::knit_child('test.txt', quiet = TRUE)
cat(res, sep = '\n')
...
The following exercise uses the idea to exercise the perception of correlation values as outlined in the [ims-tutorial](https://openintro.shinyapps.io/ims-03-model-02/#section-perception-of-correlation). 
...
my_cor <- get(params$data) %>%
    summarize(N = n(), r = cor(get(params$x), get(params$y)))
my_cor$r
...
ggplot(get(params$data), aes(get(params$x), get(params$y))) +
  geom_point() +
  xlab(params$x) +
  ylab(params$y)
